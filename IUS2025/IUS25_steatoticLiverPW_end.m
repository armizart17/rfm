...
% General directory
baseDir     = 'Q:\emiranda\bfmLiverPW2608\';
% caseData    = 'PlaneWave25cm2p5MHz';
caseData    = 'PlaneWave20cm3MHz';


% Sample and Reference directories
sampleDir   = fullfile(baseDir,caseData,'bf/');

% Outcomes directory

resultsOut  = ['RFM_', 'vIUS', caseData,'_out'];
figuresOut  = ['RFM_', 'vIUS', caseData,'_fig'];

polFolder   = 'polarFig';

resultsDir  = fullfile(baseDir,caseData,resultsOut);
figsDir     = fullfile(baseDir,caseData,figuresOut);
figsPolDir  = fullfile(baseDir,caseData,figuresOut, polFolder);
...
% ROI MANUAL 
roi_already     = true;
roisDir         = fullfile(baseDir,['rois_', caseData]);
if ~exist(roisDir) mkdir(roisDir); end

range_bmode     = [-55 0];
range_depth     = [0 19.5]; % 19.5 23.5
range_acs       = [0 1.2];
transparency    = 0.65;
...
SAM     = load(fullfile(sampleDir,samName+".mat")); % PW

% Only continue if this file is in steatoticNames
...
% hold off
% set(ax1, 'fontsize', fontSizePolar);
...
for idx = 1:2
    ...
    % xlim([-9 9])
    % ylim([range_depth(1), range_depth(2)+1])
    % ylim([0 15.5])
    hold on
    contour(xPolar*1e2, zPolar*1e2, roi,1,'w--')
    hold off
    ...
    hColor.Label.String = 'dB\cdotcm^{-1}\cdotMHz^{-1}';
    set(ax1, 'fontsize', fontSizePolar);
    set(gca, 'fontsize', fontSizePolar);
    % saveas(gcf,fullfile(figsPolDir,"acs_"+samName+"_pol"+num2str(idx)+".svg") )
    % exportgraphics(gcf,fullfile(figsPolDir,"acs_"+samName+"_pol"+num2str(idx)+".svg"))

    exportgraphics(gcf,fullfile(figsPolDir,"cbar_"+samName+"_pol"+num2str(idx)+".png"), ...
      'Resolution','300')

end

%% %%%%%%%%%%%%%%%%%%%%%% alpha %%%%%%%%%%%%%%%%%%%%%%%%
method_labels_simple = {'RFM', 'TNV-RFM'};
for idx = 1:2
