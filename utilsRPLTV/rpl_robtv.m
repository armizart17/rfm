function [u_opt,fcosts] = rpl_robtv(Y,X,Z,W,mu,u_0,params)
% function [u_opt,fcosts] = rpl_robtv(Y,X,Z,W,mu,u_0,params)
    ...
    % e_b   = norm2_fun(D*b,p*q); % € old
    % e_n   = norm2_fun(D*n,p*q); % € old
    % e_a   = norm2_fun(R*a,p*q); % € old
    ...
    % tv_b(1) = mu_b*sum(e_b); % € old
    % tv_n(1) = mu_n*sum(e_n); % € old
    % tv_a(1) = mu_a*sum(e_a); % € old
    ...
    % omega_r_b   = spdiags(kron(ones(2,1),w_b),0,omega_r_b); % € old
    % omega_r_n   = spdiags(kron(ones(2,1),w_n),0,omega_r_n); % € old
    % omega_r_a   = spdiags(kron(ones(2,1),w_a),0,omega_r_a); % € old
    ...
    while e > tol
        ...
        % e_b   = norm2_fun((omega_s_b.^0.5)*D*b,p*q); % € old
        % e_n   = norm2_fun((omega_s_n.^0.5)*D*n,p*q); % € old
        % e_a   = norm2_fun((omega_s_a.^0.5)*R*a,p*q); % € old
        ...
        % tv_b(k+1) = mu_b*sum(e_b); % € old
        % tv_n(k+1) = mu_n*sum(e_n); % € old
        % tv_a(k+1) = mu_a*sum(e_a); % € old
