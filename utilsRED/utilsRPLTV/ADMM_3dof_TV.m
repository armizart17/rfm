function [u_opt, ite] = ADMM_3dof_TV(A1, A2, A3, b, mu1, mu2, mu3, m, n, tol, mask)
    ...
    while abs(error) > tol && ite < 50
        ...
        rho = 1;
        % Update B
        B = IRLS_TV(b - A2*C - A3*N - D - v, A1, mu1/rho, m, n, tol, mask, minimask);

        % Update C
        C = IRLS_TV(b - A1*B - A3*N - D - v, A2, mu2/rho, m, n, tol, mask, minimask);

        % Update N
        N = IRLS_TV(b - A1*B - A2*C - D - v, A3, mu3/rho, m, n, tol, mask, minimask);
