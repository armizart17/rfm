...
reguType = 'normal'; % normal, high, low
...
numPhantomRef = list_phantoms(list_phantoms ~= numPhantomSam);
...
% pars.bw          = [1.9, 4]; % [MHz]
pars.bw          = [1.6 5.95]; % [MHz] % TUFFC
% pars.bw          = [1.6 5.65];
% pars.bw          = [1.6 5.5];
pars.overlap     = 0.8;
pars.blocksize   = 10; % wavelengths
% pars.z_roi       = [3 10]*1E-2; % [m] % original
% pars.x_roi       = [-5 5]*1E-2; % [m] % original

pars.z_roi       = [1.5 10]*1E-2; % [m] 
pars.x_roi       = [-6.5 6.5]*1E-2; % [m] 
...
%% 2DOF n prior delta imooverlay plots
...
loglog(band, BSC_3dof, '-', 'LineWidth', lw+1, 'DisplayName', '3-DoF'), hold on
loglog(band, BSC_2dof, '-', 'LineWidth', lw+1, 'DisplayName', '2-DoF');
loglog(band, BSC_gt, 'k-.', 'LineWidth', lw, 'DisplayName','Ground truth'),
grid on;
...
%%%%%%%%%% BSC 3DOF METRICS %%%%%%%%%%
metrics3dof.rmse    = rmse(BSC_2dof, BSC_gt);
metrics3dof.rmse_dB = rmse(BSC_2dof, BSC_gt);
metrics3dof.diff_dB = mean( abs(BSC_2dof_dB - BSC_gt_dB) ); 
