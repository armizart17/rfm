...
%% LOAD DATA
pathData = './NewPL_data/';
folderDataSam = 'data_ok';
folderDataRef = 'data_ok';
...
% fprintf('ref_a%.1fg%.2fs%.4f\n', a_sam, b_sam, n_sam);
rf_sam_name = ['ref_a',num2str(alpha_sam),'b',num2str(b_sam),'n',num2str(n_sam)];
...
%%%%%%%%%% REQUIRES AUX BECAUSE DATA INCOMPLETE %%%%%%%%%%
AUX = load(fullfile(pathData, folderDataSam, 'ref0')); % JUST AUXILIAR TO GIVE AXIS
SAM.x = AUX.x;
SAM.z = AUX.z;
SAM.fs = AUX.fs;
%%%%%%%%%% REQUIRES AUX BECAUSE DATA INCOMPLETE %%%%%%%%%%

rf_ref_name = ['ref_a',num2str(alpha_ref),'b',num2str(b_ref),'n',num2str(n_ref)];
...
if strcmp( estim_method, '2-DoF-a')
    ...
    if (methodsRegu); mu_rpl_tv    = [1E3; 10^3.5; 10^4]; % [mu_b, mu_n, mu_a]
