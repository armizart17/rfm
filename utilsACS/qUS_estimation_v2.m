function [Est_n_sample, Est_b_sample, Est_alphaz_sample,delta_SNR,SNR,est_CF,DATA,Xaxis,Zaxis,SR,z,...
    ...
    x1, x2, y1, y2, c0, BW, tissue, phantom, line_ROI_lateral, lambda_axial, estim_method, delta_flag, DATA, delta_n, ...
    struct, delta_b, delta_alpha, a_ref)
% ---------------------------------------------------------------
...
%     delta_n = -1.5; %2.8; %-6; % 2.5 4 nsam-nref
    ...
    comp_ref = comp_ref_b_bsc(log(delta_b));
...
%     mu_a  = 1e+3;%1e+3; %1e+3; % ORIGINALS
    ...
    n_ratio = reshape(n,p,q);
    alpha_ratio  = reshape(8.686*Dy*a./dz(:),p,q);
    b_ratio = delta_b*ones(size(n_ratio));
    

    elseif estim_method == 2 %% a prior
    %% a prior
...
%     delta_n = -1.5; %2.8; %-6; % 2.5 4 nsam-nref
    ...
    % Int(delta_alpha, z_0, z) = delta_a ----> delta_alpha = constant
    % delta_alpha * (z - z_0) / 8.686 = delta_a
    delta_a = delta_alpha*(z(:) - Z_ROI(1) ) / 8.686;
%     delta_a = delta_alpha*(z(:) - z(1,1) ) / 8.686;

%     comp_ref = comp_ref_a(delta_a,1,band',Z_ROI',q); % transpose to make sense
    
    comp_ref = comp_ref_a(-delta_alpha, 1, band', 0.01*Z_ROI', q); % should be in [m]
    comp_freq_a = comp_mod_freq_a(a_ref, 1, 1, band', Z_ROI', q);
...
%     mu_a  = 1e+3;%1e+3; %1e+3; % ORIGINALS
    ...
    b_ratio = reshape(b,p,q);
    n_ratio = reshape(n,p,q);
    alpha_ratio = delta_alpha*ones(size(b_ratio));
