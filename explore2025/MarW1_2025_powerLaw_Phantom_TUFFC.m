...
% bw = [2 5]; %delta_b = 10.97 dB % delta_n = 0.1341;
% bw= [1.6500 4.7500];
bw = [-Inf Inf]; %delta_b = 10.4656 dB % delta_n = 0.1941;
...
[alpha_sam, beta_sam, y_lin_sam, ~] = fit_linear(FREQ, Np2dB*ATT, 2);  % [dB/cm/MHz]

fprintf('P20 ACS sample = %.4f dB/cm/MHz\n', alpha_sam);

figure, 
set(gcf, 'Units', 'pixels', 'Position', [100, 100, 1000, 600]); % [x, y, width, height] in pixels
sgtitle('\bf AC')

subplot(1,2,1)
plottitle = sprintf('Measurements: %.2g.f^{%.2g} + %.2g', alpha, beta, n);
plot(FREQ,  ACC, 'b', 'DisplayName', plottitle)
hold on
plottitle = sprintf('Linear Fit: %.2g.f %.2g', alpha_sam, beta_sam);
plot(FREQ, y_lin_sam, 'k--', 'DisplayName', plottitle)
grid on;
xlabel('Freq [MHz]'), ylabel('AC [dB\cdotcm^{-1}]')
...
plot(FREQ, y_lin_sam, 'k--', 'DisplayName', plottitle)
grid on;
xlabel('Freq [MHz]'), ylabel('Att [dB\cdotcm^{-1}]')
legend('Location','Best')
ylim([0 5]);
title('REF: P22')
set(gca,'fontsize',fontSize)
...
idx = find(freqs_P20 >= bw(1) & freqs_P20 <= bw(2));
freqs_P20 = freqs_P20(idx);
bsc_P20   = bsc_P20(idx);

idx = find(freqs_P22 >= bw(1) & freqs_P22 <= bw(2));
freqs_P22 = freqs_P22(idx);
bsc_P22   = bsc_P22(idx);

% BSC_gt = interp1(freqs_P20, bsc_P20, freq); 

% P20
% Power Law Comparison: (bsc = b. f^n)
% Linearize: log (bsc) = n.log(f) + log(b) 
coeffs_pl   = polyfit(log(freqs_P20), log(bsc_P20), 1); % Fit y = m.x + c
n_pl        = coeffs_pl(1); % Slope = d_n  (mean),  (median)
b_pl        = exp(coeffs_pl(2)); % Intercept = ln(b) 
bsc_fit_powlaw = b_pl*freqs_P20.^n_pl;

% Display results
fprintf('-----P20 PowerLaw (b.f^n)-----\n')
fprintf('n      = %f\n', n_pl);
fprintf('b      = %e\n', b_pl);
fprintf('b [dB] = %f\n', 10*log10(b_pl));
fprintf('---------\n')
...
subplot(1,2,1)
semilogy(freqs_P20, bsc_P20, 'b','DisplayName', 'Measurements')
hold on, grid on;
plottitle = sprintf('PowLaw Fit %.3g.f^{%.3g}', b_pl, n_pl);
semilogy(freqs_P20, bsc_fit_powlaw, 'k--', 'DisplayName', plottitle);
xlabel('Freq [MHz]'), ylabel('BSC [cm^{-1}\cdot sr^{-1}]')
...
% Power Law Comparison: (bsc = b. f^n)
% Linearize: log (bsc) = n.log(f) + log(b) 
coeffs_pl   = polyfit(log(freqs_P22), log(bsc_P22), 1); % Fit y = m.x + c
n_pl        = coeffs_pl(1); % Slope = d_n  (mean),  (median)
b_pl        = exp(coeffs_pl(2)); % Intercept = ln(b) 
bsc_fit_powlaw = b_pl*freqs_P22.^n_pl;
% Display results
fprintf('-----P22 PowerLaw (b.f^n)-----\n')
fprintf('n      = %f\n', n_pl);
fprintf('b      = %e\n', b_pl);
fprintf('b [dB] = %f\n', 10*log10(b_pl));
fprintf('---------\n')
...
subplot(1,2,2)
semilogy(freqs_P22, bsc_P22, 'r', 'DisplayName', 'Measurements')
hold on, grid on;
plottitle = sprintf('PowLaw Fit %.3g.f^{%.3g}', b_pl, n_pl);
semilogy(freqs_P22, bsc_fit_powlaw, 'k--', 'DisplayName', plottitle);
xlabel('Freq [MHz]'), ylabel('BSC [cm^{-1}\cdot sr^{-1}]')
legend('Location','Southeast')
title('REF: P22');
ylim([10^-7, 10^-1])
...
numPhantomSam = 20;
j_sam = 1.107857152971458; % P20
j_ref = 1.151897288058939;
...
if numPhantomSam == 20 % P20 ACS = 0.6851 dB/cm/MHz
    b_sam       = 5.705978E-06;
    n_sam       = 3.864606;       
    alpha_sam   = 0.551509191332541; % 

elseif numPhantomSam == 22 % P22 ACS = 0.3007 dB/cm/MHz
    b_sam       = 5.125957E-07;
    n_sam       = 3.670544;
    alpha_sam   = 0.222127445307033; % 
end
...
% Power Law Comparison: (bsc = b. f^n)
% Linearize: log (bsc) = n.log(f) + log(b) 
coeffs_pl   = polyfit(log(freq), log(BSC_gt), 1); % Fit y = m.x + c
n_pl        = coeffs_pl(1); % Slope = d_n  (mean),  (median)
b_pl        = exp(coeffs_pl(2)); % Intercept = ln(b) 
bsc_fit_powlaw = b_pl*freq.^n_pl;
...
m_3dof.nrmse = sqrt(mean( ( (bsc_results{2, 1} - BSC_gt)./ BSC_gt  ).^2) );
...
m_2dofa.nrmse = sqrt(mean( ( (bsc_results{2, 2} - BSC_gt)./ BSC_gt  ).^2) );
...
m_2dofb.nrmse = sqrt(mean( ( (bsc_results{2, 3} - BSC_gt)./ BSC_gt  ).^2) );
...
m_2dofn.nrmse = sqrt(mean( ( (bsc_results{2, 4} - BSC_gt)./ BSC_gt  ).^2) );
