...
% Robust
par_rpl.m_est   = 1;
par_rpl.c       = [0.25, 25, 1E3, 1E3]; % c_fid, c_b, c_n, c_a
...
mu_rpl_tv    = [10^1.75, 1E4, 1E4]; % [mu_b, mu_n, mu_a]
...
elseif strcmp(methodEstim, 'robust')
    % SOLVE Rob RPL 3DOF
    par_rpl.df_op = 0;
    [u_opt,~] = rpl_robtv(Y,X,Z,W,mu_rpl_tv,u_0,par_rpl);
    % % [u_opt] = rpl_robust_tv(Y,X,Z,W,mu_rpl_tv, par_rpl.c, par_rpl.sigma, u_0, par_rpl);
...
tBmode = nexttile;
    ...
    title(['Bmode T', num2str(numTarget)])
    axis("image");
