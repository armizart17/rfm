function generateBMode(fileName, presetName, ax)
...
%         rfChannel(:, :, :) = getOrderedData(channelData, Receive, nSamples, nElements, nPulses, 1);
    ...
    bModeImage(:, :) = getBModeC52v(rfChannel(:, :, :), rxSamples, apodAperture, nSamples, nPulses, nElements, plotDepth, r);
...
% Get BMode
function bModeImage = getBModeC52v(rf_channel, rx_samples, apodAperture, n_samples, n_pulses, n_elements, plotDepth, tgcr)
    ...
    for n = 1:n_pulses
        ...
        % Apodization
        rf_channel(:, :, n) = rf_channel(:, :, n) .* apodAperture(n, :);0
        % Summing
