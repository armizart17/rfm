...
resultsOut  = 'res_oldFat_RFM';
figuresOut  = 'fig_oldFat_RFM';
...
sampleFiles = dir(fullfile(sampleDir,'42460445_IHR_F.mat')); %65*ma
...
if ( strcmp(samName, '42460445_IHR_F') )
...
RRSp_k               = zeros(m,n,p);
RRSp_k(:,:, 2:end)   = spectralData_sam.Sfull(:,:, 2:end) ./ spectralData_sam.Sfull(:,:, 1:end-1); 
RRSp_k(:,:, 1)       = RRSp_k(:,:, 2); % Assuming the first slice ratios are 1 as there is no "i-1"
RRSp_k               = log(RRSp_k); % @@
...
RRSp_r               = zeros(m_r,n,p);
RRSp_r(:,:, 2:end)   = spectralData_ref.Sfull(:,:, 2:end) ./ spectralData_ref.Sfull(:,:, 1:end-1); 
RRSp_r(:,:, 1)       = RRSp_r(:,:, 2); % Assuming the first slice ratios are 1 as there is no "i-1"
RRSp_r               = log(RRSp_r); % @@
...
RSp_k_ufr   = RRSp_k(:,:,range);
RSp_r_ufr   = RRSp_r(:,:,range);
