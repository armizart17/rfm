...
% kWaveArray class.
% C:\Users\armiz\AppData\Roaming\MathWorks\MATLAB Add-Ons\Collections\k-Wave
%
...
tiledlayout(1,3),
nexttile,
imagesc(1e2*rx(1,:),1e2*rz(:,1),medium.sound_speed)
...
nexttile,
imagesc(1e2*rx(1,:),1e2*rz(:,1),medium.density)
xlabel('x [cm]'), ylabel('z [cm]')
title('Density')
...
nexttile,
imagesc(1e2*rx(1,:),1e2*rz(:,1),medium.alpha_coeff)
xlabel('x [cm]'), ylabel('z [cm]')
title('Absorption')
...
figure;
imagesc(1e3 * latAxis, 1e3 * axAxis, Bmode, [-60,0]);
ylabel('Axial [mm]');
xlabel('Lateral [mm]');
axis image;
title('Intensity');
