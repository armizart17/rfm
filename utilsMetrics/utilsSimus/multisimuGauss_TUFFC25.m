function multisimuGauss_TUFFC25(alpha_power, alpha_coeff_sam, alpha_coeff_ref, numSim, nameFolderOut)
% function multisimuGauss_TUFFC25(alpha_power, alpha_coeff_sam, alpha_coeff_ref, numSim, nameFolderOut)
...
if ~exist(OutputDir,"dir"); mkdir(OutputDir); disp('FolderOut created'); end
...
%%
for iSim = 1:numSim
clearvars -except iSim alpha_power alpha_coeff_sam alpha_coeff_ref numSim nameFolderOut ...
    gen_sam gen_ref OutputDir
...
% Create the filename
filename = sprintf('rfsam%d_a_%.1f', iSim, alpha_coeff_sam);

% Save file
save(fullfile(OutputDir, filename+".mat"), "rf", "x", "z", "fs", "medium");
end
...
filename = sprintf('rfref%d_a_%.1f', iSim, alpha_coeff_ref);

% Save file
save(fullfile(OutputDir, filename), "rf", "x", "z", "fs", "medium");
