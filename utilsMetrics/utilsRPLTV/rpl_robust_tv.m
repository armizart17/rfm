function [u_opt,fcosts] = rpl_robust_tv(Y,X,Z,W,mu,c,sigma,u_0,par)
    ...
    tv_b(1) = mu_b*sum(e_b); %0.5*mu_b*(norm(e_b))^2;
    tv_n(1) = mu_n*sum(e_n); %0.5*mu_n*(norm(e_n))^2;
    tv_a(1) = mu_a*sum(e_a); %0.5*mu_a*(norm(e_a))^2;
    ...
    omega_r_n   = spdiags(kron(ones(2,1),w_n),0,omega_r_n);
    ...
    while e > tol
        ...
        e_fid = X*b+Z*n+W*a - y;
        %e_b   = norm2_fun(D*b,p*q); %(omega_s_b.^0.5)*D*b;
        %e_n   = norm2_fun(D*n,p*q); %(omega_s_n.^0.5)*D*n;
        %e_a   = norm2_fun(R*a,p*q); %(omega_s_a.^0.5)*R*a;
        e_b   = norm2_fun((omega_s_b.^0.5)*D*b,p*q);
        e_n   = norm2_fun((omega_s_n.^0.5)*D*n,p*q);
        e_a   = norm2_fun((omega_s_a.^0.5)*R*a,p*q);

        fid(k+1)  = 0.5*(norm(e_fid))^2;
        tv_b(k+1) = mu_b*sum(e_b); %0.5*mu_b*(norm((omega_r_b.^0.5)*e_b))^2;
        tv_n(k+1) = mu_n*sum(e_n); %0.5*mu_n*(norm((omega_r_n.^0.5)*e_n))^2;
        tv_a(k+1) = mu_a*sum(e_a); %0.5*mu_a*(norm((omega_r_a.^0.5)*e_a))^2;
...
% function u_opt = rpl_robust_tv(Y,X,Z,W,mu,c,sigma,par)
% 
%     if nargin < 6
%         tol   = 1e-8;
%         kmax  = 100;
...
% function u_opt = rpl_robust_tv(Y,X,Z,W,mu,c,sigma,u_0,par)
% 
%     if nargin < 6
%         tol   = 1e-8;
%         kmax  = 100;
