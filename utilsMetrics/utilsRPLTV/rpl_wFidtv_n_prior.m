function [u_opt,fcosts] = rpl_wFidtv_n_prior(Y,X,W,mu,u_0,params)
% function [u_opt,fcosts] = rpl_wFidtv_n_prior(Y,X,W,mu,u_0,params)
    ...
    % Weights in Fidelity
    w_flatten = weight(:);
    w_fid = repmat(w_flatten, r, 1); % expand for each channel
    
    omega_q_fid = spdiags(w_fid,0,omega_q_fid);
    
    % No weights in regularization
    o_flatten = ones(size(w_flatten));
    w_b = repmat(o_flatten, 2, 1);
    w_a = repmat(o_flatten, 2, 1);
