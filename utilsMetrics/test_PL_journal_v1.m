...
AUX = load(fullfile(pathData,'ref0.mat'));

% REF = load(['.\QUS_codes\NewPL_data\ref_a0.6b0.01n1.5.mat']);
SAM = load(fullfile(pathData, 'ref_a0.4b1n0.mat'));
SAM.rf = SAM.rfnew;
SAM.x = AUX.x;
SAM.z = AUX.z;
SAM.fs = AUX.fs;

REF = load(fullfile(pathData, 'ref_a0.6b0.01n1.5.mat'));
...
figure, 
subplot (1,2,1)
% imagesc(SAM.x,SAM.z, my_RF2Bmode(SAM.rf), caxis_bmode), colormap gray

imagesc( my_RF2Bmode(SAM.rfnew), caxis_bmode), colormap gray
title(['Bmode SAM: ', ])

subplot (1,2,2)
imagesc( my_RF2Bmode(REF.rfnew), caxis_bmode), colormap gray
title('Bmode REF')
